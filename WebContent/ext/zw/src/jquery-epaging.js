...
;(function(factory) {
    ...
    var defaults = {
        //按钮html
        pagingBtnHtml: '<a class="{{class}}" data-type="{{type}}" href="{{link}}">{{text}}</a>\n',
        //页码输入框html
        pagingInputHtml: '<input class="{{class}}" type="text" />\n',
        //按钮跳转
        linkTo: '#',
        //按钮样式
        pagingBtnCls: '',
        ...
        if(me.settings.store.total && me.settings.store.total !== 0){
            ...
            me.$doms.each(function(index, el) {
                ...
                if(me.settings.store.curPage <= 4){
                    for(i = 1; i <= me.settings.store.totalPage && btnNums < 5; i++){
                        if (i === me.settings.store.curPage) {
                            //当前页
                            $this.append(me.getPageBtn({ text: i, class: me.settings.pagingActiveCls, type: 'number', disabled: true }));
                        } else {
                            //其他页
                    ...
                    for(i = me.settings.store.curPage - 2 - lessNums; i <= me.settings.store.totalPage && btnNums < 5; i++){
                        if (i === me.settings.store.curPage) {
                            //当前页
                            $this.append(me.getPageBtn({ text: i, class: me.settings.pagingActiveCls, type: 'number' }));
                        } else {
                            //其他页
                            $this.append(me.getPageBtn({ text: i, type: 'number' }));
                        }
                ...
                if (me.settings.store.curPage !== 1) {
                    ...
                    $this.prepend(me.getPageBtn({ text: '上一页', type: 'prev', class: me.settings.btnDisabledCls, disabled: true }));
                }
                ...
                if (me.settings.store.curPage !== me.settings.store.totalPage) {
                    ...
                    $this.append(me.getPageBtn({ text: '下一页', type: 'next', class: me.settings.btnDisabledCls, disabled: true }));
                }

                //加入跳转控件
                if(me.settings.enablePageJump === true){
                    var $input = $(me.tpl(me.settings.pagingInputHtml, { class: me.settings.pagingInputCls }));
